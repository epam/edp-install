    ...
    openshift.withCluster() {
        openshift.withProject() {
            if (!openshift.selector("buildconfig", "${vars.itemMap.name}").exists())
                openshift.newBuild("--name=${vars.itemMap.name}", "--image-stream=s2i-${vars.itemMap.language.toLowerCase()}", "--env DOTNET_STARTUP_PROJECT=${vars.deployableModule}", "--binary=true")
            buildResult = openshift.selector("bc", "${vars.itemMap.name}").startBuild("--from-dir=${vars.workDir}", "--wait=true")
