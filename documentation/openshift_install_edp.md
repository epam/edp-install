...
* For EDP, it is required to have Keycloak access to perform integration. To do this, create 
manually a secret with an administrative access username and a password or use the existing secret. 
According to our templates, it is possible to execute the following secret export command:

```bash
oc -n <edp_main_keycloak_project> get secret keycloak --export -o yaml | oc -n <edp_cicd_project> apply -f -
```
