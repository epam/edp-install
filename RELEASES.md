...
## Version 3.10.0 <a name="3.10.0"></a> (October 11, 2024)
...
Our platform now integrates with Large Language Models, such as ChatGPT, as an intelligent [chat assistant](https://docs.kuberocketci.io/docs/next/user-guide/add-ai-assistant). Depending on your specified settings, it can perform a variety of tasks. We can also set up this assistant for you. Please contact our team for more details.
...
We have added the ability to specify custom build and review pipelines with just one click when editing or creating new codebase branches. This enhancement increases flexibility when working with codebases, allowing for easier modifications to pipeline logic based on specific requirements.
...
Users can now deliver their applications in the form of RPM packages, implemented using Ansible. Correspondingly, support for C and C++ languages with make and CMake build tools has been added to the platform.
...
Additionally, we have added new functionality to our deployment pipelines. A new tab called Variables has been added to the Environment (formerly Stage) details page. In this tab, users can specify key-value parameters, which will be transmitted within the application as environment variables. This improvement allows users to configure application deployment settings without leaving the KubeRocketCI portal.

Deploy pipelines now support manual approval. When creating a custom deploy pipeline, users can include a checkpoint to prompt for approval or rejection before proceeding to the next step.

We have added a new deployment trigger type called "[Auto-stable](https://github.com/epam/edp-cd-pipeline-operator/blob/master/deploy-templates/crds/v2.edp.epam.com_stages.yaml#L161)" has been added to the platform. This trigger type allows for creating an ordered queue of deploy Pipeline Runs, which will be launched sequentially once the previous one finishes. Unlike the "Auto" trigger type, where intermediate pipelines are skipped, "Auto-stable" ensures that each version is deployed, making it easier to identify the exact application version that caused errors during deployment.
...
* The platform has been verified on the [1.29](https://github.com/kubernetes/kubernetes/blob/master/CHANGELOG/CHANGELOG-1.30.md#v1300) version. ([#66](https://github.com/epam/edp-cd-pipeline-operator/issues/66))
...
General:
- EDP is renamed to KubeRocetCI throughout docs. ([#278](https://github.com/epam/edp-install/issues/278))
- Update KubeRocketCI names and documentation links | The README.md file of the edp-install repostory has been updated. ([#273](https://github.com/epam/edp-install/issues/273))


- Replace EDP with KubeRocketCI in README md ([#268](https://github.com/epam/edp-tekton/issues/268))
- Add asset for the documentation | Troubleshooting section updated ([#214](https://github.com/epam/edp-install/issues/214))
...
- Add clean env behaviour | When creating an environment (formerly stage), users can select the clean pipeline. ([#317](https://github.com/epam/edp-headlamp/issues/317))
